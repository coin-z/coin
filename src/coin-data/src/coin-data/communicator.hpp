...
private:
    static std::shared_ptr< std::map<std::string, MutableCommunicator::Ptr> > mutable_communicator();

    static Service<MutableServerReq, MutableServerAck>::Ptr mutable_communicator_service_;

    static void registerMutableCommunicator_(const std::string& id, const MutableCommunicator::Ptr& mc);
    static bool mutableCommunicatorServiceCallback_(
...
public:
    ...
    void setSubCallback(const typename Subscriber<DataT>::Callback& cb)
    {
        sub_callback_ = cb;
        if(not sub_name_.empty())
        {
            std::cout << "set sub callback to " << sub_name_ << std::endl;
...
inline typename MutableWriter<DataT>::Ptr MutableWriter<DataT>::create(const std::string &id)
{
    ...
    Communicator::registerMutableCommunicator_(id, obj);
    return obj;
}
