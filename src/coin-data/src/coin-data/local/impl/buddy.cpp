...
void buddy_memory_check(void* addr, const size_t size)
{
    ...
    for(size_t l = 0; l < level; l++)
    {
        ...
        {
            struct block_head_t* b = (struct block_head_t*)(addr + total_size * idx);
            ...
            printf("addr: %lx, size: %ld(%lx), ", addr, size, size);
            printf("check level: %ld, step: %ld, ", l, step);
            printf("check addr: %lx\n", b);
